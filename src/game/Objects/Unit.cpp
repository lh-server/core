...
{
    if (IsPlayer() && GetXZFlagBasedSpeed(this) > LEEWAY_MIN_MOVE_SPEED && GetXZFlagBasedSpeed(target) > LEEWAY_MIN_MOVE_SPEED)
...
{
    if (IsPlayer() && (GetXZFlagBasedSpeed(this) > LEEWAY_MIN_MOVE_SPEED || ToPlayer()->m_movementInfo.HasMovementFlag(MOVEFLAG_JUMPING)))
        return LEEWAY_BONUS_RANGE;
